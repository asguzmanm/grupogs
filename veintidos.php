...
<body>
    ...
    //Consultar los 8 del Nivel 3 (Quienes pagan)
    $records_3 = $conn->prepare('SELECT DISTINCT nivel_3 FROM usuarios WHERE nivel_6=:numero');
    $records_3->bindParam(':numero', $_SESSION['numero']);
    $records_3->execute();

    //Consultar si tiene personas que le paguen    
    $personas = $conn->prepare('SELECT COUNT(DISTINCT nivel_3) AS cantidad FROM usuarios WHERE nivel_6=:numero');
    ...
    $saldorecibido = $conn->prepare('SELECT sum(valor) AS recibido from pago22 WHERE realizado="Si" AND phone_destino=:destino');
    ...
    $saldoconfirmado = $conn->prepare('SELECT sum(valor) AS confirmado from pago22 WHERE realizado="Si" AND confirmado="Si" AND phone_destino=:destino');
    ...
    //Consultar el numero de Nivel 9 (a quien debe pagarle)
    $records_6 = $conn->prepare('SELECT DISTINCT nivel_9 FROM usuarios WHERE nivel_6=:numero');
    $records_6->bindParam(':numero', $_SESSION['numero']);
    $records_6->execute();
    $results_6 = $records_6->fetch(PDO::FETCH_ASSOC);
    $numero_nivel_6 = $results_6['nivel_9'];
    //Finaliza Consultar el numero de Nivel 9

    //Consultar el Nombre y Nequi de Nivel 9
    $records_61 = $conn->prepare('SELECT name, nequi, pin FROM usuarios WHERE phone=:numero_3');
    $records_61->bindParam(':numero_3', $numero_nivel_6);
    $records_61->execute();
    $results_61 = $records_61->fetch(PDO::FETCH_ASSOC);
    $nombre_nivel_9 = $results_61['name'];
    $nequi_nivel_9 = $results_61['nequi'];
    $pin_nivel_9 = $results_61['pin'];
    //Finaliza Consultar el Nombre y Nequi de Nivel 9
    ...
    <form action="" method="POST" id="form" enctype="multipart/form-data">
        ...
        <div class="contenedor">
            ...
            ?>
                ...
                <div class="grupo">
                    ...
                    while ($results_3 = $records_3->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        //Consultar el pago y valor de cada uno de los 8 del Nivel 3 (Quienes pagan)
                        require 'db.php';
                        $confirmacion = $conn->prepare('SELECT realizado, valor FROM pago22 WHERE phone=:phone');
                        $confirmacion->bindParam(':phone', $results_3['nivel_3']);
                        ...
                        //Consultar el nombre de cada uno de los 8 del Nivel 3 (Quienes pagan)
                        $nombre_n3 = $conn->prepare('SELECT name, pin FROM usuarios WHERE phone=:phone');
                        $nombre_n3->bindParam(':phone', $results_3['nivel_3']);
                        $nombre_n3->execute();
                    ...
                    ?>
                            <div class="info-consignacion">
                                <label class="hola" for=""><?php echo $resultado_nombre_n3['name'] . "<br>" . "Cel. " . $results_3['nivel_3'] . "<br>"; ?></label>
                                <label class="pago-hecho" for=""><?php echo "$" . number_format($estado_valor, 0) ?></label>
                                <a href='evidencia.php?.<?php echo codifica_urlget('previa=veintidos.php&basedatos=pago22&numerofrom=' . $results_3['nivel_3']) ?>' target="_blank" onclick="window.open(this.href,this.target,'width=360,height=720,top=200,left=200,toolbar=no,location=no,status=no,menubar=no');return false;">Ver</a>

                                <?php
                                require 'db.php';
                                $confirmacion_recibido = $conn->prepare('SELECT confirmado FROM pago22 WHERE phone=:phone');
                                $confirmacion_recibido->bindParam(':phone', $results_3['nivel_3']);
                                ...
                                ?>
                                    ...
                                    <a class="confirmar-pago" href='confirmar_pago.php?.<?php echo codifica_urlget('previa=veintidos.php&basedatosp=pago22&basedatosr=pago22&numero=' . $results_3['nivel_3'] . '&nombre_orig=' . $nombre_orig . '&nombre_cliente=' . $nombreCliente . '&cel_cliente=' . $numeroCliente . '&valor=' . $valor . '&motivo=' . $motivo . '&pin_orig=' . $pin_orig) ?>'>Confirmar</a>
                                <?php
                                }
                ...
                if ($numero_n_3Cliente === '0') { ?>
                    <div class="pay-made-no">
                        ...
                        <div>
                            <label class="ganancia-texto" for="">Por ser cabeza de equipo, no debes realizar devolución de dinero</label>
                        </div>
                    </div>
                <?php
                }
                ...
                ?>
                    ...
                    <div class="cabecera-info">
                        <label class="ganancia-texto1" for="">Una vez hayas confirmado las consignaciones,<br>realiza la devolución del 50% equivalente a</label><br>
                        <label class="devolucion" for=""><?php echo "$" . number_format($saldor / 2, 0) ?></label><br>
                        <label class="ganancia-texto1" for="">al Nequi de la siguiente persona:</label>
                    </div>
                    ...
                    <div class="grupo">
                        <div class="info-consignacion11">
                            ...
                            <div class="textos">
                                <label class="hola" for=""><?php echo $nombre_nivel_9 ?></label><br>
                                <label class="numero" for="">Cel. <?php echo $numero_nivel_6 ?></label><br>
                                <label class="nombre" for="">Nequi: <?php echo $nequi_nivel_9 ?></label><br>
                            </div>
                        </div>
                    </div>
                    ...
                    $confirmacion = $conn->prepare('SELECT realizado, dateconfirm FROM pago33 WHERE phone=:numero');
                    ...
                    $recibido = $conn->prepare('SELECT confirmado, daterecibe FROM pago33 WHERE phone=:numero');
                    ...
                    ?>
                        ...
                        if ($estado_recibido === 'No') { ?>
                            <div class="pay-made-no">
                                ...
                                <div>
                                    <label class="ganancia-texto" for="">La consignación pronto sera confirmada <br> por <?php echo $nombre_nivel_9 ?></label><br>
                                </div>
                            </div>
                        <?php
                            ...
                            <div class="pay-made">
                                <img src="images/double_check.png" class="img-pay-made-time" alt="">
                                <div>
                                    <label class="ganancia-texto" for="">La consignación fue confirmada por <br> <?php echo $nombre_nivel_9 . " el<br>" . date("d-M-Y g:i a", strtotime(($resultado_recibido['daterecibe']))) ?></label><br>
                                </div>
                            </div>
                        <?php
                        }
                        ...
                        <button name="boton_confirmar_pago_22" onclick="return confirm('Confirmar consignación');">Confirmar Consignación</button><b>&nbsp</b>

            <?php

                        if (isset($_REQUEST['boton_confirmar_pago_22'])) {
                            ...
                            if (isset($_FILES['image']['name'])) {
                                ...
                                echo $nombre = $_SESSION['numero'] . "_to_" . $numero_nivel_6 . "_pago22." . $img_type;
                                ...
                                if ($img_type === 'PNG' || $img_type === 'png') {
                                    ...
                                    $sql = $conn->prepare('INSERT INTO pago33 (phone, phone_destino, valor, dateconfirm, realizado, daterecibe, confirmado, evidencia) VALUES (:phone,:numero_nivel_3,:valor,:fecha,:realizado,:comodin,:confirmado,:nombre)');
                                    $sql->bindParam(':phone', $phone);
                                    $sql->bindParam(':numero_nivel_3', $numero_nivel_6);
                                    ...
                                    if ($sql->execute()) {
                                        include 'variables.php';
                                        notificar_devolucion_veintidos(ucwords($nombre_nivel_9), "33 (Díe de 3er Pago)", $nombreCliente, $numeroCliente, number_format($valor, 0), "Devolución del 50%", $pagina, $numero_nivel_6, $pin_nivel_9, $youtube_confirmar_pago_dev, $token, $url_curl);
                                        echo "<script>alert('Pago Confirmado');window.location= 'veintidos.php'</script>";
                                    } else {
                                    ...
                                    if ($sql->execute()) {
                                        include 'variables.php';
                                        notificar_devolucion_veintidos(ucwords($nombre_nivel_9), "33", $nombreCliente, $numeroCliente, number_format($valor, 0), "Devolución del 50%", $pagina, $numero_nivel_6, $pin_nivel_9, $youtube_confirmar_pago_dev, $token, $url_curl);
                                        echo "<script>alert('Pago Confirmado');window.location= 'veintidos.php'</script>";
                                    } else {
                                        $message = 'Ocurrio un error, intenta más tarde';
                                    }
            ...
            function notificar_devolucion_veintidos($nombre_dest, $dia, $nombre, $cel, $valor, $motivo, $pagina, $celular, $PIN, $video, $token, $url_curl)
            {
                $curl = curl_init();

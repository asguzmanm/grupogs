...
<body>
    ...
    //Consultar los 8 del Nivel 0 (Quienes pagan)
    $records_3 = $conn->prepare('SELECT phone, name, pin FROM usuarios WHERE nivel_3=:numero');
    $records_3->bindParam(':numero', $_SESSION['numero']);
    $records_3->execute();

    //Consultar si tiene personas que le paguen    
    $personas = $conn->prepare('SELECT COUNT(*) AS cantidad FROM usuarios WHERE nivel_3=:numero');
    ...
    $saldorecibido = $conn->prepare('SELECT sum(valor) AS recibido from pago11 WHERE realizado="Si" AND phone_destino=:destino');
    ...
    $saldoconfirmado = $conn->prepare('SELECT sum(valor) AS confirmado from pago11 WHERE realizado="Si" AND confirmado="Si" AND phone_destino=:destino');
    ...
    //Consultar el numero de Nivel 6 (a quien debe pagarle)
    $records_6 = $conn->prepare('SELECT nivel_6 FROM usuarios WHERE nivel_3=:numero');
    $records_6->bindParam(':numero', $_SESSION['numero']);
    $records_6->execute();
    $results_6 = $records_6->fetch(PDO::FETCH_ASSOC);
    $numero_nivel_6 = $results_6['nivel_6'];
    //Finaliza Consultar el numero de Nivel 6

    //Consultar el Nombre y Nequi de Nivel 6
    ...
    $nombre_nivel_6 = $results_61['name'];
    $nequi_nivel_6 = $results_61['nequi'];
    $pin_nivel_6 = $results_61['pin'];
    //Finaliza Consultar el Nombre y Nequi de Nivel 6
    ...
    <form action="" method="POST" id="form" enctype="multipart/form-data">
        ...
        <div class="contenedor">
            ...
            <div class="ganancia">
                <label class="ganancia-texto" for="">Las siguientes personas deben realizarte <br> el pago de $20.000 c/u. Por favor, has clic en <br> confirmar cuando lo hayan hecho</label><br>
            </div>
            ...
            if ($numeropersonas === '0') { ?>
                <div class="pay-made-no">
                    ...
                    <div>
                        <label class="ganancia-texto" for="">Lo sentimos, no tienes personas que te consignen</label>
                    </div>
                </div>
            <?php
            }
            ...
            ?>
                ...
                <div class="grupo">
                    ...
                    while ($results_3 = $records_3->fetch(PDO::FETCH_ASSOC)) {
                        ...
                        //Consultar el pago de cada uno de los 8 del Nivel 0 (Quienes pagan)
                        require 'db.php';
                        $confirmacion = $conn->prepare('SELECT realizado FROM pago11 WHERE phone=:phone');
                        $confirmacion->bindParam(':phone', $results_3['phone']);
                    ...
                    ?>
                            <div class="info-consignacion">
                                <label class="hola" for=""><?php echo $results_3['name'] . "<br>" . "Cel. " . $results_3['phone'] . "<br>"; ?></label>
                                <label class="pago-hecho" for="">$20.000</label>
                                <a href='evidencia.php?.<?php echo codifica_urlget('previa=once.php&basedatos=pago11&numerofrom=' . $results_3['phone']) ?>' target="_blank" onclick="window.open(this.href,this.target,'width=360,height=720,top=200,left=200,toolbar=no,location=no,status=no,menubar=no');return false;">Ver</a>

                                <?php
                                require 'db.php';
                                $confirmacion_recibido = $conn->prepare('SELECT confirmado FROM pago11 WHERE phone=:phone');
                                $confirmacion_recibido->bindParam(':phone', $results_3['phone']);
                                ...
                                <?php
                                    ...
                                    $nombre_orig = $results_3['name'];
                                    $nombreCliente = $resultsCliente['name'];
                                    $numeroCliente = $resultsCliente['phone'];
                                    $valor = intval('20000');
                                    $motivo = "1er Pago";
                                    $pin_orig = $results_3['pin'];
                                    include 'variables.php';

                                ?>
                                    
                                    <a class="confirmar-pago" href='confirmar_pago.php?.<?php echo codifica_urlget('previa=once.php&basedatosp=pago11&basedatosr=pago22&numero=' . $results_3['phone'] . '&nombre_orig=' . $nombre_orig . '&nombre_cliente=' . $nombreCliente . '&cel_cliente=' . $numeroCliente . '&valor=' . $valor . '&motivo=' . $motivo . '&pin_orig=' . $pin_orig) ?>'>Confirmar</a>
                                <?php
                                }
                ...
                ?>
                    ...
                    <div class="grupo">
                        <div class="info-consignacion11">
                            ...
                            <div class="textos">
                                <label class="hola" for=""><?php echo $nombre_nivel_6 ?></label><br>
                                <label class="numero" for="">Cel. <?php echo $numero_nivel_6 ?></label><br>
                                <label class="nombre" for="">Nequi: <?php echo $nequi_nivel_6 ?></label><br>
                            </div>
                        </div>
                    </div>
                    ...
                    $confirmacion = $conn->prepare('SELECT realizado, dateconfirm FROM pago22 WHERE phone=:numero');
                    $confirmacion->bindParam(':numero', $_SESSION['numero']);
                    $confirmacion->execute();
                    $resultado = $confirmacion->fetch(PDO::FETCH_ASSOC);
                    $estado = $resultado['realizado'];

                    //Consultar si el pago fue confirmado por L6

                    $recibido = $conn->prepare('SELECT confirmado, daterecibe FROM pago22 WHERE phone=:numero');
                    ...
                    ?>
                        ...
                        if ($estado_recibido === 'No') { ?>
                            <div class="pay-made-no">
                                ...
                                <div>
                                    <label class="ganancia-texto" for="">La consignación pronto sera confirmada <br> por <?php echo $nombre_nivel_6 ?></label><br>
                                </div>
                            </div>
                        <?php
                            ...
                            <div class="pay-made">
                                <img src="images/double_check.png" class="img-pay-made-time" alt="">
                                <div>
                                    <label class="ganancia-texto" for="">La consignación fue confirmada por <br> <?php echo $nombre_nivel_6 . " el<br>" . date("d-M-Y g:i a", strtotime(($resultado_recibido['daterecibe']))) ?></label><br>
                                </div>
                            </div>
                        <?php
                        }
                        ...
                        <button name="boton_confirmar_pago_11" onclick="return confirm('Confirmar consignación');">Confirmar Consignación</button><b>&nbsp</b>

            <?php

                        if (isset($_REQUEST['boton_confirmar_pago_11'])) {
                            ...
                            if (isset($_FILES['image']['name'])) {
                                ...
                                $nombre = $_SESSION['numero'] . "_to_" . $numero_nivel_6 . "_pago11." . $img_type;
                                ...
                                if ($img_type === 'PNG' || $img_type === 'png') {
                                    ...
                                    $sql = $conn->prepare('INSERT INTO pago22 (phone, phone_destino, valor, dateconfirm, realizado, daterecibe, confirmado, evidencia) VALUES (:phone,:numero_nivel_3,:valor,:fecha,:realizado,:comodin,:confirmado,:nombre)');
                                    ...
                                    if ($sql->execute()) {
                                        include 'variables.php';
                                        notificar_devolucion_once(ucwords($nombre_nivel_6), "22 (Día de 2do Pago)", $nombreCliente, $numeroCliente, number_format($valor, 0), "Devolución del 50%", $pagina, $numero_nivel_6, $pin_nivel_6, $youtube_confirmar_pago_dev, $token, $url_curl);
                                        echo "<script>alert('Pago Confirmado');window.location= 'once.php'</script>";
                                    } else {
            ...
            function notificar_devolucion_once($nombre_dest, $dia, $nombre, $cel, $valor, $motivo, $pagina, $celular, $PIN, $video, $token, $url_curl)
            {
                $curl = curl_init();
